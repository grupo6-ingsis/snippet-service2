...
class FormatConfigServiceTests {
    ...
    @Nested
    inner class FormatSingleSnippetRequestTests {
        @Test
        fun `should create FormatSingleSnippetRequest and access properties`() {
            val snippetId = "123e4567-e89b-12d3-a456-426614174000"
            val content = "print('hello')"
            val request = FormatSingleSnippetRequest(snippetId, content)
            assertEquals(snippetId, request.snippetId)
            assertEquals(content, request.content)
            assert(request.toString().contains(snippetId))
            assert(request.toString().contains(content))
        }
    }

    @Nested
    inner class FormatConfigEntityTests {
        @Test
        fun `should set and get autogenerated id`() {
            val config = FormatConfig()
            val uuid = UUID.randomUUID()
            config.id = uuid
            assertEquals(uuid, config.id)
        }
    }
}
